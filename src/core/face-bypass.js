...
(function() {
    ...
    function showReportNotification() {
        ...
        let message = 'Someone reported us (check note).';
                    ...
                    try {
                        ...
                        if (typeof event.data === 'string' && event.data.startsWith('{')) {
                            ...
                            if (msg.event === 'rimage') {
                                ...
                                // Dispatch a raw event to be handled by content script
                                const reportEvent = new CustomEvent('chromegleReportReceived', {
                                    detail: {
                                        message: msg // Pass the whole message
                                    }
                                });
                                window.dispatchEvent(reportEvent);
                                console.log('Chromegle: Dispatched raw chromegleReportReceived event.');
                            }
